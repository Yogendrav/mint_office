...
if OccupationalGroup.all.empty?
  a = OccupationalGroup.create!(name: '정규직')
  b = OccupationalGroup.create!(name: '인턴')
  c = OccupationalGroup.create!(name: '프리랜서')
  OccupationalGroup.create!(name: '비상근임직원')

  employee = Employee.first
  employee.occupational_group = a
  employee.save!

  employee2 = Account.find_by_name('test').person.employee
  employee2.occupational_group = b
  employee2.save!


  account = Account.create!(name: 'freelance', password: '1234')
  person = account.person
  Person.current_person = person

  contact = Contact.create(firstname:'free', lastname:'lance', company_name: 'freelance')
  contact.company = company
  person.contact = contact

  employee = person.create_employee(juminno: '771122-1234231', occupational_group: c)
