...
class EmployeeTest < ActiveSupport::TestCase
  ...
  test "Employee should be searched by name and email" do
    account = Account.first
    employee = Employee.search(account, :join, "개똥")

    assert employee.present?
    assert employee.first.fullname.include?("개똥")

    employee = Employee.search(account, :join, "test@test.com")
    assert employee.present?
    assert employee.first.email.include?("test@test.com")
  end
end