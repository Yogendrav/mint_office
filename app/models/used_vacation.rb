...
class UsedVacation < ActiveRecord::Base
  ...
  validate :validates_period_step, :if => Proc.new{|used_vacation| used_vacation.period.present?}
  validate :validates_start_time, on: :create
  ...
  def title
    t = (self.type.nil? or self.vacation.nil?) ? "" : "#{self.type.title} : #{self.vacation.employee.fullname}"
    if !from_half.nil? and from == to and from_half == to_half
      t += " (" + (I18n.t ("used_vacations."+from_half)) + ")"
    end
    t
  end
  ...
  private
  def validates_period_step
    errors.add(:period, I18n.t('activerecord.errors.models.account.attributes.period.invalid')) if period % 0.5 != 0
  end

  def validates_start_time
    errors.add(:from, I18n.t('activerecord.errors.models.account.attributes.start_time.before_today')) if start_time.beginning_of_day < Time.zone.now.beginning_of_day
  end
