class CommutesController < ApplicationController
  ...
  def create
    @commute = commute
    @commute.employee = current_employee
    @commute.go!
    #Attachment.save_for(@commute, current_employee, uploaded_file: params[:uploaded_file])
    if @commute.save
      redirect_to commute_path(current_employee)
    end
  end

  def excel
    commutes = Commute
    send_file commutes.export(params[:to].to_sym)
  end
end